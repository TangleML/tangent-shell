...
#   -B, --batch           Treat SOURCE_DIR as a parent folder of bundles and run
#                         the deploy process for each immediate subfolder that
#                         contains a tangent.yaml.
...
# Bump + pack + (optionally) install a single bundle source folder. Returns
# non-zero on failure instead of exiting, so batch mode can keep going.
deploy_one() {
  local SOURCE_DIR="$1"
  local MANIFEST="$SOURCE_DIR/tangent.yaml"
  local BUNDLE_ID BUNDLE_VERSION ZIP_PATH HTTP_CODE
...
if [[ "$BATCH" -eq 0 ]]; then
  deploy_one "$SOURCE_DIR"
  exit 0
fi

# Batch mode: SOURCE_DIR is a parent folder of bundles. Deploy each immediate
# subfolder that has a tangent.yaml, continuing past failures.
if [[ ! -d "$SOURCE_DIR" ]]; then
  echo "error: batch source is not a directory: $SOURCE_DIR" >&2
  exit 1
fi
...
for dir in "$SOURCE_DIR"/*/; do
  bundle="${dir%/}"
  [[ -d "$bundle" ]] || continue
  if [[ ! -f "$bundle/tangent.yaml" ]]; then
    echo "skip: no tangent.yaml in $bundle"
    skipped+=("$bundle")
    continue
  fi
  echo "=== Deploying $bundle ==="
  if deploy_one "$bundle"; then
    deployed=$((deployed + 1))
  else
    echo "error: failed to deploy $bundle" >&2
    failures+=("$bundle")
  fi
  echo
done

echo "Batch summary: $deployed deployed, ${#failures[@]} failed, ${#skipped[@]} skipped."
