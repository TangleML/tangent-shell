...
During Phase 3, ask whether this scenario should *consume* or *produce*
...
3. **Do you have pipeline source code?** A dehydrated pipeline YAML in your
   repo (with component `name:` or `digest:` refs, not inline specs). If yes,
   ask for its path — this is preferred over exporting from a run because
   `--hydrate` on submit will always resolve the latest component versions.
   If no, we'll export from the baseline run and dehydrate it.
...
**Introspect:** Download metrics from the baseline run. Find eval tasks,
get their metric artifacts, read the JSON.
...
`inspect_component`. If BQ tables are involved, use Data Portal MCP to
discover schemas. When querying BQ, use `LIMIT 10` for previews, `LIMIT 100`
max for analysis. Select only needed columns. After large queries, summarize
findings to a file — don't leave raw results in conversation context.
...
- [ ] Asked about relevant BQ tables
- [ ] User confirmed checkpoint summary
...
pipeline:
  ...
  source_path: "<optional: path to dehydrated pipeline template in the repo>"
