...
- **Never hard-pin a single untested public URL.** Bare
  `urllib.request.urlretrieve` from Tangle often hits `HTTP 403 Forbidden`. The
  run config must instead set `dataset_uri: "auto"` with ordered fallbacks, use a
  verified accessible `gs://`/data-source URI, or document why the chosen source
  works in Tangle.
