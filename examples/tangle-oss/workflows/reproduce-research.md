...
explicit artifact wiring) and submits via the Tangle CLI (`uv run tangle sdk
pipeline-runs submit ...`). Do NOT let it collapse the DAG into one task — a
monolithic task is incorrect even if it validates/submits. It reports milestones
with the markers `BUILDER_STARTED`, `FILES_CREATED`, `CORRECTED_DAG_READY`,
`VALIDATION_STATUS`, `PIPELINE_SUBMITTED`, `SUBMISSION_FAILED`.
