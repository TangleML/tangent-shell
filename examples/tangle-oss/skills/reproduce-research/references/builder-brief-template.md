...
- **CLI**: drive the open-source Tangle CLI via Bash (`uv run tangle ...`); see
  [`.tangent/skills/tangle-cli/SKILL.md`](../../tangle-cli/SKILL.md) for install
  and the command reference. Ensure `TANGLE_API_URL` (and auth if the backend
  requires it) is set before any API-backed command.
- **Source annotation** (set on submit): pass `--annotation tangent=true` so the
  run is attributed to this workflow.
