...
- Use the read-only `tangle_*` API tools for lookups. When the task calls for
  ...
  `tangle_run_cancel`, `tangle_run_annotation_set`); for building, iterating on,
  or running pipelines, drive the open-source **Tangle CLI** (`uv run tangle ...`)
  via Bash — see the `tangle-cli` skill (`skills/tangle-cli/SKILL.md`) for
  install and commands. Do not use hosted MCP tools.
...
- The current user's identity (name + email) may be provided in your context
  under a `## Current user` heading. Use their email as the user id for Tangle
  API requests and attribution when the task calls for it.
