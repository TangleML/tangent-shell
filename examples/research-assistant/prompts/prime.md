...
- Save the full write-up as a Markdown file under `artifacts/`, e.g.
  `artifacts/<topic-slug>.md`, structured answer-first, then supporting evidence,
  then a references section.
- Keep a short inline summary in the chat alongside the link, e.g.
  `[Open report](artifacts/<topic-slug>.md)`.
- Verify the file exists and pin it before reporting done; never report a research
  task as done without the link and pin.
