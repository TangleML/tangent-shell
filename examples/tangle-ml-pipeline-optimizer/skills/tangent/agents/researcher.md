...
4. **Data Platform** — untapped BigQuery tables, label sources, signals,
   negative-mining sources. Use `LIMIT 10` for previews, `LIMIT 100` max for
   analysis. Select only needed columns and filter on partition keys. Write
   a summary file after any large query — don't leave raw BQ results in context.
   Also check whether a previous experiment **promoted** a curated eval
   set, frozen feature snapshot, or annotation dataset you could reuse —
   the scenario's `MEMORY.md` and session logs are the first place to
   look for a recorded `data_source_id`. If you have a hunch about who
   produced it, the **`Find data source`** component resolves an
   `(author, resource_name)` pair to the latest matching
   `data_source_id`. Reusing a preserved artifact beats re-deriving one.
...
8. **Slack (FYI)** — skip if unavailable.
...
Run `tangle-deploy docs pipeline` for pipeline schema details and
