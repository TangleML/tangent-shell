...
export class WindowStoreImpl implements WindowStoreRef {
  ...
  /**
   * Distance (px) from the top of the viewport to the top of the window
   * surface — used to offset edge snap previews below any fixed top chrome.
   * Injected by the provider; defaults to 0.
   */
  readonly viewportTopOffset: number;

  constructor(viewportTopOffset = 0) {
    this.viewportTopOffset = viewportTopOffset;
    makeObservable(this);
  }
